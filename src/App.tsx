...
export default function App() {
  ...
  const [screen, setScreen] = useState<'loading' | 'home' | 'modeselect' | 'game'>('loading');
  ...
  useEffect(() => {
    ...
    function handleKeyUp(e: KeyboardEvent) {
      ...
      switch (e.key) {
        ...
        case "h": case "H": dispatch({ type: "HOLD" }); break;
        case "r": case "R": if (gameModeRef.current !== 'competitive') dispatch({ type: "RESTART" }); break;
      }
    }
    ...
    function loop() {
      ...
      if (canvas) {
        ...
        if (ctx) {
          ...
          // Line-clear flash
          const lc = lineClearAnimRef.current;
          if (lc) {
          ...
          if (gameOver) {
            ...
            // Dark red-tinted overlay fades in (starts at 200ms)
            if (since > 200) {
  ...
  if (screen === 'home')       return <>{goHomeOverlay}<HomeScreen onStart={handleGoModeSelect} /></>;
    ...
    return (
      <>
        ...
        <div style={{ background: "linear-gradient(180deg, #000812 0%, #000508 100%)", minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", userSelect: "none", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 9, padding: "10px 16px 14px", width: "100%", background: "linear-gradient(180deg, transparent 0%, rgba(0,4,16,0.97) 18%)", borderTop: "1px solid rgba(0,200,255,0.07)", marginTop: 2 }}>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", width: "100%" }}>
              <HoldBtn onPress={() => dispatch({ type: "HOLD" })} canHold={state.canHold} />
              <TouchBtn onPress={() => dispatch({ type: "ROTATE" })} label="ROTATE" color="#a000f0" wide />
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", width: "100%" }}>
              <TouchBtn onPress={() => dispatch({ type: "MOVE_LEFT" })} label="◀" color="#00aaff" size={64} />
              <TouchBtn onPress={() => dispatch({ type: "MOVE_DOWN" })} label="↓" color="#00f060" size={64} />
              <TouchBtn onPress={() => dispatch({ type: "MOVE_RIGHT" })} label="▶" color="#00aaff" size={64} />
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", width: "100%" }}>
              <TouchBtn onPress={() => dispatch({ type: "HARD_DROP" })} label="DROP" color="#f0a000" wide />
              {gameMode !== 'competitive'
